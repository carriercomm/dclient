...
Array::Array (std::initializer_list<Value> const &array)
{
  UInt i = 0;
  for (Value const &v : array)
    (*this)[i++] = v;
}

Object::Object (std::initializer_list<std::pair<char const *, Value>> const &map)
{
...
void
Path::addPathInArg (const std::string &path, const InArgs &in, InArgs::const_iterator &itInArg, PathArgument::Kind kind)
{
  if (itInArg == in.end ())
    {
